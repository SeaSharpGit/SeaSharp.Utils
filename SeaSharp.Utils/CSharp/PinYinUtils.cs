#region AF.NET HZExtension：封装汉字处理类
// +----------------------------------------------------------------------
...
// 'Z'; //54481..55289 
        ...
        public const int MinValue = 45217;
        public const int MaxValue = 55289;

        private static char[] pyFirstNames = new char[]{
            'A',
            'B',
            'C',
            'D',
        ...
        private static char[] pyFirstLowerNames = new char[]{
            'a',
            'b',
            'c',
            'd',
        ...
        private static int[] pyFirstValues = new int[]{
            45217,
            45253,
            45761,
            46318,
        ...
        #region 注释（解析Z开头的文字会出问题）
        //public static string GetPYFirsts(this string hzString)
        //{
        //    return GetPYFirsts(hzString, (sb, index) => { sb.Append(pyFirstNames[index]); });
        //}

        //public static string GetPYFirstLowers(this string hzString)
        //{
        //    return GetPYFirsts(hzString, (sb, index) => { sb.Append(pyFirstLowerNames[index]); });
        //}

        //private static string GetPYFirsts(this string hzString, Action<StringBuilder, int> appendIndex)
        //{
        //    StringBuilder sb = new StringBuilder();
        //    foreach (var c in hzString)
        //    {
        //        var bytes = System.Text.Encoding.Default.GetBytes(new char[] { c });
        //        if (bytes.Length == 1)
        //        {
        //            sb.Append(c);
        //            continue;
        ...
        //        if (value >= pyFirstValues[pyFirstValues.Length - 1])
        //        {
        //            appendIndex(sb, pyFirstValues.Length - 1);
        //            continue;
        //        }

        //        var index = SearchInIndex(pyFirstValues, 0, pyFirstValues.Length - 1, value);
        //        if (index >= 0)
        //        {
        ...
        static int SearchInIndex(int[] a, int left, int right, int key)
        {
            int mid = 0; //计算当前检索的数组的中间位置 
            while (left <= right)
            {
                mid = (left + right) / 2;
                if (a[mid] == key)
                {
                    return mid;//如果中间元素与检索元素相等，则返回该元素的位置 
                }
                else if (a[mid] < key)
                {
